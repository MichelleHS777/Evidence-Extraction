...
from ZhKeyBERT.zhkeybert import KeyBERT, extract_kws_zh
...
def get_similar_sentence():
    ...
    for data in tqdm(dataset, desc='getting similar sentence...'):
        ...
        ev_sent = [s[0] for s in sent2sim[:5] if (s[1] + get_weight(claim, s[0])) > 0.8]
        print(ev_sent)
...
def get_keyword(sent):
    kw_model = KeyBERT(model='paraphrase-multilingual-MiniLM-L12-v2')
    keywords = extract_kws_zh(sent, kw_model, ngram_range=(1, 1), diversity=0.25)
    return keywords


def get_weight(claim, sent):
    score = 0
    claim_keyword = dict([key for key in get_keyword(claim)])
    claim_keyword = list(claim_keyword.items())
    claim_keyword.sort(key=lambda s: s[1], reverse=True)
    claim_keyword = set(claim_keyword[:5])
    sent_list = set(jieba.cut(sent, cut_all=False))
    # print('claim keyword:', claim_keyword)
    # print('sent words:', sent_list)
    for key in claim_keyword:
        if key[0] in sent_list:
            score += key[1]
    return score
