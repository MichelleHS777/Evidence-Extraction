...
def main():
    ...
    for data in tqdm(dataset, desc='getting similar sentence...'):
        ...
        # with open('evidencesExtract/similarityValuesA.json', 'a+', encoding='utf-8') as f:
        #     tmp = json.dumps(sims, ensure_ascii=False)
        #     print(tmp, file=f)
    with open('evidencesExtract/similarityValues.json', 'w', encoding='utf-8') as f:
        json.dump(similarityValues, f, indent=2, ensure_ascii=False)

    similaritys_list = json.load(open('datasets/evidencesExtract/similarityValues.json', 'r', encoding='utf-8'))
    isgold_list = json.load(open('datasets/evidencesExtract/isgoldLabel.json', 'r', encoding='utf-8'))
    tfidfvalues_list = json.load(open('datasets/evidencesExtract/tfidfValues.json', 'r', encoding='utf-8'))
    X_trans = np.array([])
    y_trans = np.array([])
    for i in tqdm(range(len(isgold_list))):
        ...
        y = [ev['isgold'] for ev in obj['evidences']]
    ...
    for i in tqdm(range(len(isgold_list))):
        ...
        try:
            ...
            low += len(obj['evidences'])
            top5 = []
            for j in rank[-5:][::-1]:
                top5.append(obj['evidences'][j]['text'])
...
def labelevidences():
    ...
    for row in tqdm(data_list):
        ev_sents = []
        # ev_sents += re.split(r'[？：。！.“”…\t\n]', row['content'])
        for ev in row['evidences'].values():
            ev_sents += re.split(r'[？：。！.“”…\t\n]', ev['text'])
        ...
        for ev_sent in ev_sents:
            ev_isgold[ev_sent] = 0
            for ev in row['gold evidences'].values():
                gold_evidences = ev['text']
                if len(gold_evidences) == 0:
                    continue
                c1 = Counter(ev_sent)
                c2 = Counter(gold_evidences)
                miss_num = 0
                for character, count in c2.items():
                    miss_num += abs(count - c1[character])
                if miss_num / len(gold_evidences) < 0.3:
                    ev_isgold[ev_sent] = 1
        for sent, val in ev_isgold.items():
            obj['evidences'].append({
                'text': sent,
                'isgold': val
            })
        label_evidencess.append(obj)
    with open('evidencesExtract/isgoldLabel.json', 'w', encoding='utf-8') as f:
        json.dump(label_evidencess, f, indent=2, ensure_ascii=False)
...
def countTfIDF(sents_list):
    ...
    with open('evidencesExtract/tfidfValues.json', 'w', encoding='utf-8') as f:
